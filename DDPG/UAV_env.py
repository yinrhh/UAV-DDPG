...
class UAVEnv(object):
    ...
    slot_time = 10  # 时间帧时长
    tao_time = 2.5  # 时隙时长
    step_punish = -20  # 跳过step惩罚
    step_reward = 20  # 跳过step奖励
    ...
    task_list = np.random.randint(2000000, 2000001, M)  # 随机生成终端任务大小：2MB
    ...
    def step(self, action, eps):
        ...
        ################# UE各项参数 ######################
        task_size_ratio = action[2:]  # UE卸载的任务比例
        task_size = np.zeros(self.M)  # UE卸载的任务量
        # 卸载比例转换为真正处理的任务大小。
        for i in range(self.M):
            ...
            if t_loc_cal > self.tao_time:
                task_size[i] = 0
                reward += self.step_punish * task_size_ratio[i]
            else:
    ...
    def record_step(self, energy, x, y, uav_task_size, local_task_size, t_loc_off, t_loc_cal, task_size_ratio, eps):
        ...
        with open(file_name, 'a') as file_obj:
            ...
            file_obj.write("\ntask_size_ratio: " + '{}'.format(task_size_ratio))
